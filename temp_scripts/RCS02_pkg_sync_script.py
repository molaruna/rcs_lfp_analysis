...
pkg_dir = '/Users/mariaolaru/Documents/temp/RCS02/RCS02L_pkg_data/'
...
#fp_psd = '/Users/mariaolaru/Documents/temp/RCS02/RCS02L/RCS02L_pre-stim/RCS02L_pre-stim_psd_periodic.csv'
fp_psd = '/Users/mariaolaru/Documents/temp/RCS02/RCS02L/RCS02L_pre-stim/RCS02L_pre-stim_psd_aperiodic.csv'
fp_coh = '/Users/mariaolaru/Documents/temp/RCS02/RCS02L/RCS02L_pre-stim/RCS02L_pre-stim_coh.csv'
...
title = "RCS02 PKG-RCS pre-stim time-series sync"
...
plt.plot(np.arange(1, len(df)+1, 1), df["('" + keyword + "'," + str(freq) + ".0,'" + contacts[1] + "')"], alpha = 0.7, label = str(freq)+ "Hz " + contacts[1], markersize = 1, color = 'mediumpurple')
...
#plt.hlines(0.03, 0, len(df), alpha = 0.7, label = 'SVM thresh', color = 'red')

plt.plot(np.arange(1, len(df)+1, 1), df['BK'], alpha = 0.7, label = 'PKG-BK', markersize = 1, color = 'indianred')
#plt.plot(np.arange(1, len(df)+1, 1), df['DK'], alpha = 0.7, label = 'PKG-DK', markersize = 1, color = 'steelblue')
...
#run SVM
sync.run_svm_wrapper(df_merged, keyword, 'DK', 0.03)
sync.run_svm_wrapper(df_merged, 'Cxy', 'DK', 0.03)
sync.run_svm_wrapper(df_merged_combo, '+', 'DK', 0.03)
sync.run_svm_wrapper(df_merged_combo, keyword, 'DK', 0.03)
sync.run_svm_wrapper(df_merged_combo, 'Cxy', 'DK', 0.03)
sync.run_svm_wrapper(df_merged_combo, "+3-1')", 'DK', 0.03)
sync.run_svm_wrapper(df_merged_combo, "+10-8')", 'DK', 0.03)
sync.run_svm_wrapper(df_merged_combo, '+3-1_+10-8', 'DK', 0.03)

#run linear regressions
sync.run_lm_wrapper(df_merged, keyword, 'DK', 2) #all spectra channels
...
sync.run_lm_wrapper(df_merged_combo, keyword, 'DK', 2) #2 spectra channels
...
#get top features
#df_coefs_s = df_coefs.loc[:, ['features', str(0.03)]]
#df_imp = df_coefs_s.sort_values(by = [str(0.03)]).iloc[::-1].reset_index(drop=True)
#df_imp[str(0.03)] = df_imp[str(0.03)].round(3)
...
#features = np.append(df_imp['features'][0:10], df_imp['features'][len(df_imp)-10:len(df_imp)])
...
#sklearn.model_selection.cross_val_score(LinearDiscriminantAnalysis(), X, y)
